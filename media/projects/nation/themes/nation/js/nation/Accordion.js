	...
	function _initClass() {
		...
		Accordion.prototype.openPanel = function(index, immediate) {
			...
			if (immediate) {
				...
				if (!this.jQueryMode) {
					...
					NATION.Animation.start(panel, {height: newHeight}, {duration: this.options.duration, easing: this.options.easing}, this.onPanelOpenComplete.bind(this));
				} else {
					if (this.options.autoScroll) {
						$(window).stop().animate({scrollTop: headerOffset}, this.options.duration, this.options.easing, this.onAutoScrollComplete.bind(this));
					}
		...
		Accordion.prototype.closePanel = function(index, immediate) {
			...
			if (immediate) {
				...
				if (!this.jQueryMode) {
					...
					NATION.Animation.start(panel, {height: 0}, {duration: this.options.duration, easing: this.options.easing}, this.onPanelCloseComplete.bind(this));
				} else {
					$(panel).stop().animate({height: 0}, this.options.duration, this.options.easing, function(e) {this.onPanelCloseComplete(e.target);});
				}
			}
		}
		...
		Accordion.prototype.onHeaderClicked = function(e) {
			if (this.enabled) {
				...
				if (this.openPanelID >= 0) {
					this.closePanel(this.openPanelID, !this.options.animateTransitions);
				}
		...
		*/
		Accordion.prototype.onPanelOpenComplete = function(e) {
			if (this.options.maxPanelHeight < 0) {
