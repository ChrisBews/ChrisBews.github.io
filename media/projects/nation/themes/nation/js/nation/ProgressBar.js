...
(function(window, document, undefined) {
	...
	function _initClass() {
		...
		* @param {string} handlePositioning Either "outside" (default) or "inside". Defines how the handle should be positioned within the bar. "outside" sees the cenre of the handle move between 0 and 100% of the bar, thus leading to it poking outside. "inside" keeps the handle fully within the bar
		* @jsFiddle //jsfiddle.net/NationStudio/7thLue5y/embedded/
		*/
		var ProgressBar = function(selector, handlePositioning) {
			NATION.EventDispatcher.call(this);
			this.handlePositioning = handlePositioning;
			if (!handlePositioning) this.handlePositioning = "outside";
		...
		*/
		ProgressBar.prototype.setProgress = function(percentage) {
			...
			if (this.handlePositioning === "inside") {
		...
		*/
		ProgressBar.prototype.updateHandleOnDrag = function(xPos) {
			...
			if (this.handlePositioning === "inside") {
				...
				//if (this.__hitArea) {
				//	if (xPos > this.__hitArea.offsetWidth - this.__handle.offsetWidth) {
				//		xPos = this.__hitArea.offsetWidth - this.__handle.offsetWidth;
				//	}
				//} else {
					if (xPos > this.__DOMElement.offsetWidth - this.__handle.offsetWidth) {
						//xPos = this.__DOMElement.offsetWidth - this.__handle.offsetWidth;
					}
				//}
			}
			var percentage = xPos / progressBarWidth;
		...
		*/
		ProgressBar.prototype.onHandleTouchStart = function(e) {
			...
			this.handler_TouchEnd = this.HandleTouchEnd.bind(this);
